...
|---|---|---|---|---|---|
| Claude | `ClaudeCredentialsReader`: `~/.claude/.credentials.json`, then cross-app Keychain; `ClaudeCLIUsageProbe` fallback | Success, 5-hour window | Success, 5-hour window | Complete on this already-authorized Mac; fresh-install reliability still needs OAuth/API key or explicit folder grant | **High**: success may depend on an existing Keychain ACL; CLI fallback launches an external tool |
| Codex | `CodexAuthReader`: `~/.codex/auth.json`; `CodexUsageService`: local session JSONL fallback | Success, 7-day window | Failed as “not logged in” | **Needs one-time folder authorization or OAuth** | **High**: hidden config/session access; the current silent automatic path does not survive Sandbox |
...
| Local history/trends (`ccusage`) | `CCUsageService`: `/bin/zsh` + `npx ccusage` | Success | Helper process failed | **Sandbox-unavailable in current design** | **High**: external shell/package execution; this breaks a promised free local feature |
