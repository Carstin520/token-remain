...
const TRAY_DISPLAY_MODES = ["full", "compact", "minimal"];
...
/// This window hides to the tray rather than closing, so a mounted glass
/// surface would otherwise keep an SVG displacement filter and a backdrop
/// filter resident on a window nobody is looking at.
...
function App() {
  ...
  const glassActive = useWindowVisible();
  ...
  return (
    // The dashboard window is opaque, so the glass here layers over the app's
    // own surfaces and carries no popup preference: it stays on the shipped
    // Frosted defaults rather than following the popover's appearance panel.
    <GlassProvider active={glassActive}>
...
function Sidebar({ state, section, onSelect, onRefresh, onOpenPopup, onOpenUpdate }) {
  return (
      ...
      <div className="sidebar-footer">
        {state.availableUpdate ? (
          <GlassSurface
            ...
            onClick={() => onOpenUpdate(state.availableUpdate.url)}
            title={trKey("update.install_help")}
            aria-label={trKey("update.available_accessibility", [state.availableUpdate.version])}
          >
            <ArrowUpRightIcon />
            <span>{trKey("update.available_format", [state.availableUpdate.version])}</span>
          </GlassSurface>
        ) : null}
...
function SyncFooter({ state, onRefresh, onOpenPopup }) {
  ...
  return (
    <GlassSurface role="card" className="sync-footer">
...
function Overview({ state, action, onSelect, onOpen }) {
  ...
  return (
    <section className="content-section overview-section">
      ...
      <AIFeed state={state} action={action} onOpen={onOpen} />
    </section>
  );
}

/// The KPI row is the one part of the dashboard body that takes glass: four
/// small tiles reading as their own pane above the flat panels below them.
/// Non-interactive, so the row costs no per-frame work while the cursor crosses
/// it on the way to the content.
function MetricCard({ label, value, caption, valueTone, captionTone }) {
  return (
    <GlassSurface as="article" role="card" interactive={false} className="metric-card">
      ...
      {hasEntries ? (
        <>
          <div className="usage-composition">
            <div className="donut" style={{ "--ring-fill": `conic-gradient(${stops})` }}>
              <div>
      ...
      {ranked.length ? (
        <>
          <div className="official-quota-rows">
            {ranked.map((id) => (
              <OfficialQuotaRow key={id} id={id} provider={state.providers.find((item) => item.providerID === id)} strategy={state.summaryStrategy} />
            ))}
          </div>
...
function OfficialQuotaRow({ id, provider, strategy }) {
  const meta = providerPresentation(id);
  const window = summaryWindow(provider, strategy);
...
function AIFeed({ state, action, onOpen }) {
  ...
  return (
    <div className="ai-feed">
      ...
      />
      <div className="settings-card feed-notification-card">
        <PanelHeader
          title={tr("Notifications")}
          subtitle={tr(state.feedNotificationsEnabled ? "Quota resets and major updates arrive as system notifications" : "Notifications are off")}
          trailing={(
            <button
              className={`switch ${state.feedNotificationsEnabled ? "on" : ""}`}
              role="switch"
              aria-checked={Boolean(state.feedNotificationsEnabled)}
              aria-label={tr("Important-update notifications")}
              onClick={() => action(() => api.setFeedNotificationsEnabled(!state.feedNotificationsEnabled))}
            >
              <span className="knob" />
            </button>
          )}
...
function QuotaCard({ provider, notice, noticeDetail, id, preferences, canRemove, onRemove }) {
  ...
  const detailRows = providerQuotaDetailRows(provider);
  return (
    <article className="provider-card" title={tr("Drag the card to reorder")}>
      ...
      {windows.length ? (
        ...
        <div className="empty-provider">
          <MoonIcon />
          <span title={notice ? (noticeDetail || notice) : undefined}>{notice || tr("%1$@ is waiting for its Windows-local sign-in or credential.", [meta.name])}</span>
        </div>
      )}
    </article>
  );
      ...
      <div className="settings-card source-list">
        ...
        {enabled.length ? enabled.map((definition) => definition.access === "local-credential" ? (
          ...
          <SourceHealthRow
            ...
            detailTitle={state.noticeDetails?.[definition.id]?.detail}
...
function GeneralSettings({ state, action }) {
  ...
  const trayProviders = state.trayProviders || ["claude", "codex"];
  const trayProviderSet = new Set(trayProviders);
  const trayProviderLimitReached = trayProviders.length >= 4;
  const toggleTrayProvider = (providerID) => {
    const nextSet = new Set(trayProviders);
    if (nextSet.has(providerID)) nextSet.delete(providerID);
    else if (!trayProviderLimitReached) nextSet.add(providerID);
  ...
  return (
    <div className="settings-card">
      ...
      <Divider />
      <div className="preference-row language-setting">
        <label className="preference-label" htmlFor="summary-strategy">
          <strong>{trKey("settings.quota_summary_title")}</strong>
          <span>{trKey("settings.quota_summary_hint")}</span>
        </label>
        <select
          id="summary-strategy"
          className="language-select"
          value={state.summaryStrategy || "shortestWindow"}
          onChange={(event) => action(() => api.setSummaryStrategy(event.target.value))}
        >
          <option value="shortestWindow">{trKey("settings.quota_summary_shortest")}</option>
          <option value="lowestRemaining">{trKey("settings.quota_summary_tightest")}</option>
        </select>
      </div>
      <Divider />
      <div className="preference-row tray-mode-setting">
        <div className="preference-label">
          <strong>{tr("Tray icon")}</strong>
          <span>{tr("Show live quota in the Windows tray as a number, two rings, or one ring.")}</span>
        </div>
        <div className="tray-mode-segmented" role="radiogroup" aria-label={trKey("settings.menubar_mode_title")}>
          {TRAY_DISPLAY_MODES.map((mode) => (
            <button
              type="button"
              key={mode}
              role="radio"
              aria-checked={state.trayDisplayMode === mode}
              className={state.trayDisplayMode === mode ? "selected" : ""}
              onClick={() => action(() => api.setTrayDisplayMode(mode))}
            >
              {trKey(`settings.menubar_mode_${mode}`)}
            </button>
          ))}
        </div>
      </div>
      <div className="tray-provider-setting">
        <div className="preference-label">
          <strong>{tr("Tray providers")}</strong>
          <span>{tr("Choose up to 4 providers; compact mode draws the first two selected providers.")}</span>
        </div>
        <div className="tray-provider-chips">
          {(state.providerCatalog || []).map((provider) => {
            const meta = providerPresentation(provider.id);
            const selected = trayProviderSet.has(provider.id);
            return (
              <button
                type="button"
                key={provider.id}
                ...
                disabled={!selected && trayProviderLimitReached}
                onClick={() => toggleTrayProvider(provider.id)}
              >
                <ProviderMark meta={meta} size={16} />
                <span>{meta.name}</span>
                <CheckCircleIcon />
              </button>
            );
  ...
  return (
    <>
      <div className="settings-card">
        ...
        <div className="preference-row language-setting">
          <label className="preference-label" htmlFor="refresh-interval">
            <strong>{tr("Refresh interval")}</strong>
            <span>{tr("How often each provider API is queried automatically; \"Manual only\" queries only when you click refresh")}</span>
          </label>
          <select
            id="refresh-interval"
            className="language-select"
            value={state.refreshMinutes ?? 5}
            onChange={(event) => action(() => api.setRefreshMinutes(Number(event.target.value)))}
          >
            {REFRESH_OPTIONS.map((option) => <option key={option.value} value={option.value}>{tr(option.label)}</option>)}
          </select>
        </div>
        <Divider />
...
function createPreviewAPI() {
  ...
  const previewLocalProviders = [
    {
      ...
      planName: "Max 20x",
      windows: [{ usedPercent: 43, windowMinutes: 300, resetsAt: previewNow + 10_320_000 }, { usedPercent: 18, windowMinutes: 10_080, resetsAt: previewNow + 421_200_000 }],
      extraUsage: { spentUSD: 12.5, monthlyLimitUSD: 50 },
      scopedWindows: [{ scopeID: "fable", displayName: "Fable", window: { usedPercent: 72, windowMinutes: 10_080, resetsAt: previewNow + 270_000_000 } }],
    },
    {
      providerID: "codex",
      capturedAt: previewNow - 12 * 60_000,
      planName: "Pro 5x",
      windows: [{ usedPercent: 63, windowMinutes: 300, resetsAt: previewNow + 9_000_000 }, { usedPercent: 31, windowMinutes: 10_080, resetsAt: previewNow + 331_200_000 }],
      codexResetCredits: { availableCount: 2 },
      scopedWindows: [{ scopeID: "codex_bengalfox", displayName: "GPT-5.3-Codex-Spark", window: { usedPercent: 35, windowMinutes: 10_080, resetsAt: previewNow + 400_000_000 } }],
    },
  ];
  ...
  return {
    ...
    setPopoverGlassStyle: async (value) => (preview = { ...preview, popoverGlassStyle: value }),
    setPopoverBackdropOpacity: async (value) => (preview = { ...preview, popoverBackdropOpacity: value }),
