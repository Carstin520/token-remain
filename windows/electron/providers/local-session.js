...
export async function collectCursor({ env = process.env, now = Date.now(), fetchImpl = fetch } = {}) {
  ...
  if (!auth) throw signInRequiredError("Cursor is not signed in on this PC");
  if ((decodeJWTExpiry(auth.accessToken) || Infinity) <= now) throw signInRequiredError("Cursor sign-in is stale; open Cursor once to renew it");
...
export async function collectCopilot({ env = process.env, now = Date.now(), fetchImpl = fetch } = {}) {
  ...
  if (!token) throw signInRequiredError("GitHub Copilot is not signed in on this PC");
...
async function collectCodeium(providerID, { env = process.env, now = Date.now(), fetchImpl = fetch } = {}) {
  ...
  if (!auth) throw signInRequiredError(`${titleCase(providerID)} is not signed in on this PC`);
...
export async function collectGrok({ env = process.env, now = Date.now(), fetchImpl = fetch } = {}) {
  ...
  if (!auth) throw signInRequiredError("Grok CLI is not signed in on this PC");
  if (auth.expiry && auth.expiry <= now) throw signInRequiredError("Grok sign-in is stale; run Grok once to renew it");
...
export function parseKiroUsage(raw, now = Date.now()) {
  ...
  if (/not logged in|login required|kiro-cli login|oauth error/i.test(text)) throw signInRequiredError("Kiro CLI is not signed in");
