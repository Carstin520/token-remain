...
} from "../codex-snapshots.js";
import { noticeFromError, signInRequiredError } from "../notification-policy.js";
...
export async function collectCodex({ env = process.env, now = Date.now(), fetchImpl = fetch, snapshotCache } = {}) {
  const home = resolveCodexHome(env);
  let quota;
  let apiError;
  try {
    quota = await collectCodexAPI({ home, now, fetchImpl });
  } catch (error) {
    apiError = normalizedCodexAPIError(error);
  }
  if (apiError) {
    try {
      const local = await collectCodexLocalSnapshot(home, snapshotCache ? { cache: snapshotCache } : undefined);
      return { ...local, collectorNotice: noticeFromError(apiError) };
    } catch {
      throw apiError;
    }
  }

  // The account API remains authoritative for its primary/secondary windows.
  // Local token_count rows only add active model-pool limits between API reads.
  try {
    const local = await collectCodexLocalSnapshot(home, snapshotCache ? { cache: snapshotCache } : undefined);
    const scopedWindows = local.scopedWindows?.filter((scope) => scope.window.resetsAt === undefined
      || scope.window.resetsAt > quota.capturedAt) || [];
    if (scopedWindows.length) return { ...quota, scopedWindows };
  } catch {}
  return quota;
}

async function collectCodexAPI({ home, now, fetchImpl }) {
...
function normalizedCodexAPIError(error) {
  if (error?.name === "TimeoutError" || /aborted due to timeout/i.test(error?.message || "")) {
    return new Error("Codex quota request timed out. Check the Windows proxy or firewall, then refresh.");
  }
  if (/Request failed \((401|403)\)/.test(error?.message || "")) {
    return signInRequiredError("Codex sign-in was rejected. Run codex logout, sign in again, then refresh.");
  }
  return error;
}
...
export { codexModelScopedWindows, codexWireScopeBase, parseCodexSessionSnapshots };
