...
test("Scoped windows join the expanded card with Fable enabled by default", () => {
  const [card] = popoverQuotaCards({ providers: [{ providerID: "claude", capturedAt: NOW, windows: [
    { usedPercent: 43, windowMinutes: 300, resetsAt: NOW + 3 * 3_600_000 },
  ], scopedWindows: [
  ...
  assert.equal(card.scopedWindows.length, 3);
  ...
  const [fableHidden] = popoverQuotaCards({
    providers: [{ providerID: "claude", capturedAt: NOW, windows: [
      { usedPercent: 43, windowMinutes: 300, resetsAt: NOW + 3 * 3_600_000 },
    ], scopedWindows: [
      { scopeID: "fable", displayName: "Fable", window: { usedPercent: 10, windowMinutes: 10_080 } },
...
test("Quota cards are capped and skip providers with no snapshot", () => {
  ...
  const [card] = popoverQuotaCards(
    {
      ...
      notices: { claude: "Claude usage read timed out; showing the latest cached data" },
      noticeDetails: {
        claude: {
          message: "Claude usage read timed out; showing the latest cached data",
          kind: "timeout",
  ...
  assert.equal(card.notice, "Claude usage read timed out; showing the latest cached data");
  assert.equal(card.noticeKind, "timeout");
  assert.equal(card.noticeDetail, "net::ERR_CONNECTION_TIMED_OUT");
});
...
test("Usage digest fills Today, Yesterday, and Last 30 Days from the synced aggregate", () => {
  ...
  // Each legend row quotes its own provider's recorded cost — the ring's hover
  // label and the row tooltip both read it from here, never from a split of
  // the day's total.
  assert.deepEqual(digest.entries.map((entry) => entry.cost), [1.5, 0.5]);
  assert.ok(digest.entries.every((entry) => entry.hasCompletePricing));
});

test("A provider's cost is withheld when its own tokens went unpriced", () => {
  const digest = buildUsageDigest({ sourceDay: "2026-08-07", capturedAt: NOW, days: [{
    day: "2026-08-07",
    agents: [
      { id: "claude", tokens: 2_000, cost: 1.25 },
      // Priced models plus one ccusage has no price for: the dollar figure is
      // real but incomplete, so the row must not present it as the whole cost.
      { id: "codex", tokens: 1_000, cost: 0.4, unpricedModels: ["gpt-experimental"] },
      // Tokens with no cost at all are the other incomplete case…
      { id: "gemini", tokens: 500, cost: 0 },
      // …except for a local model, which genuinely costs nothing.
      { id: "ollama", tokens: 900, cost: 0 },
    ],
  }] }, NOW);
  const byID = Object.fromEntries(digest.entries.map((entry) => [entry.id, entry]));
  assert.equal(byID.claude.cost, 1.25);
  assert.equal(byID.claude.hasCompletePricing, true);
  assert.equal(byID.codex.cost, 0.4);
  assert.equal(byID.codex.hasCompletePricing, false);
  assert.equal(byID.gemini.hasCompletePricing, false);
  assert.equal(byID.ollama.cost, 0);
  assert.equal(byID.ollama.hasCompletePricing, true);
});
