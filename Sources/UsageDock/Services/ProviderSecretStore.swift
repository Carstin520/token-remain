...
struct ProviderSecretStore {
    ...
    static let descriptors: [Descriptor] = [
        Descriptor(provider: .deepseek,
                   ...
                   placeholder: "粘贴 DeepSeek API Key(platform.deepseek.com → API Keys）",
                   isCookie: false),
        Descriptor(provider: .kimi,
                   envKeys: ["KIMI_CODE_API_KEY", "KIMI_API_KEY"],
                   placeholder: "粘贴 Kimi Code API Key 或 kimi-auth Cookie 值",
                   isCookie: false),
        Descriptor(provider: .minimax,
                   envKeys: ["MINIMAX_API_KEY"],
                   placeholder: "粘贴 MiniMax API Key（Coding Plan 账户）",
                   isCookie: false),
        Descriptor(provider: .mimo,
                   envKeys: ["MIMO_COOKIE"],
                   placeholder: "粘贴 platform.xiaomimimo.com 登录后的 Cookie",
                   isCookie: true),
        Descriptor(provider: .qoder,
                   envKeys: ["QODER_COOKIE"],
                   placeholder: "粘贴 qoder.com 登录后的 Cookie",
                   isCookie: true),
        Descriptor(provider: .volcengine,
                   envKeys: ["VOLCENGINE_ACCESS_KEY"],
                   placeholder: "粘贴火山引擎 AccessKeyId:SecretAccessKey（冒号分隔）",
                   isCookie: false),
        Descriptor(provider: .ollama,
                   envKeys: ["OLLAMA_COOKIE"],
                   placeholder: "粘贴 ollama.com 登录后的 Cookie（含 session-token）",
                   isCookie: true)
    ]
